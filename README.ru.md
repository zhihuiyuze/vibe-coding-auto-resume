...
Чтобы посмотреть, что есть, со временем и началом последнего user-сообщения каждой сессии:

```bash
vibe history                               # 10 самых свежих для текущего cwd
vibe history --limit 0                     # все
vibe history --json                        # для пайпа в jq / скрипты
```
...
Берёте нужный UUID и `vibe run --resume <uuid>`.
...
`vibe ls` показывает все `vibe-*` tmux-сессии с их текущим cwd, статусом аттача и пометкой `← here`, когда cwd совпадает с вашим:
...
- **N совпадений** → интерактивный picker (`1..N` чтобы выбрать, `n` для новой).

Явный `vibe work <name>` пропускает обнаружение — имя всегда выигрывает.

**Совет по именованию**: для проектов, к которым возвращаетесь часто, давайте сессии явное имя (`vibe work projectA`) вместо хэша cwd. Это переживает переименования путей и symlinks, и читаемо в выводе `vibe ls`.
